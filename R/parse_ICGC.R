...
parse_ICGC <- function() {
  ...
  PACA_CA_samplesheet_fname <- system.file("extdata/ICGC",
                                           "sample.PACA-CA.1478119211159.tsv",
                                           package = "pdac")
  ...
  PACA_CA_samplesheet_seq <- PACA_CA_samplesheet_seq[!duplicated(PACA_CA_samplesheet_seq),]
  ...
  PACA_CA_exp_seq_fname <- system.file("extdata/ICGC",
                                       "PACA-CA.exp_seq.tsv.gz",
                                       package = "pdac")
  PACA_CA_exp_seq <- read.table(gzfile(PACA_CA_exp_seq_fname ),
                                sep = "\t",
                                header = TRUE)
  PACA_CA_seq <- list()
  PACA_CA_seq$ex <- dcast(data = PACA_CA_exp_seq,
  ...
  PACA_CA_seq$sampInfo <- data.frame(icgc_sample_id = names(PACA_CA_seq$ex))
  PACA_CA_seq$sampInfo <- cbind(PACA_CA_seq$sampInfo,
                                PACA_CA_samplesheet_seq[match(PACA_CA_seq$sampInfo$icgc_sample_id,
                                                              PACA_CA_samplesheet_seq$icgc_sample_id),
  ...
  PACA_AU_seq <- list()
  PACA_AU_seq$ex <- dcast(data = PACA_AU_exp_seq,
                          formula = gene_id ~ icgc_sample_id,
                          value.var = "normalized_read_count",
                          fun.aggregate = sum)
  ...
  PACA_AU_array <- list()
  PACA_AU_array$ex <- dcast(data = PACA_AU_exp_array,
                          formula = gene_id ~ icgc_sample_id,
  ...
  # change this so that 1 = no and 0 = yes
  ...
  PACA_CA_seq$metadata <- list(log.transformed = FALSE,
                               ...
                               accession = "PACA-CA",
                               description = "43 cell lines, 5 xenografts, and one pancreatic primary tumor",
                               survivalA = "None",
                               survivalB = "None",
                               exp.type = "RNAseq")
  ...
  # ------------------------------------------------------------------------------------
  save(list = c("PACA_AU_array"),
       file = "./data/PACA_AU_portal_array.RData")
  save(list = c("PACA_AU_seq"),
       file = "./data/PACA_AU_portal_seq.RData")
  save(list = c("PACA_CA_seq"),
       file = "./data/PACA_CA_portal_seq.RData")
